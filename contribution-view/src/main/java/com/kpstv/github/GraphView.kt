...
class GraphView @JvmOverloads constructor(
    ...
    var graphData: List<GraphModel> = emptyList()
        set(value) {
            field = value
            requestLayout()
        }
    ...
    init {
        context.withStyledAttributes(attrs, R.styleable.GraphView, defStyleAttr) {
            commitTextPaint.color = getColor(R.styleable.GraphView_textColor, if (isInEditMode) Color.BLACK else Color.LTGRAY)
            timelineTextPaint.color = getColor(R.styleable.GraphView_textColor, if (isInEditMode) Color.BLACK else Color.LTGRAY)
            linePaint.color = getColor(R.styleable.GraphView_lineColor, if (isInEditMode) Color.BLACK else Color.LTGRAY)
            graphPaint.color = getColor(R.styleable.GraphView_graphColor, 0xC8517558.toInt())

            linePaint.strokeWidth = getDimension(R.styleable.GraphView_lineThickness, 1f)
            commitTextPaint.textSize = getDimension(R.styleable.GraphView_commitTextSize, 13.sp())
            timelineTextPaint.textSize = getDimension(R.styleable.GraphView_timelineTextSize, 11.sp())
        }
    }
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        graphData.forEachIndexed { i, gm ->
            val maxY = abs(maxHeight - (lineDistance * gm.c / commits))
            ...
            if (exceptionalCase || startX >= (c + 1) * lineDist.toInt()) {
                ...
                lineDatesUnix.add(Date(gm.w * 1000))
                c++
            }
            startX += widthSpacing
        }
